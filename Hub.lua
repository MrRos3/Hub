...
local function iconButton(parent, name, image, position, size)
    local button = create("ImageButton", {
        Name = name,
        BackgroundColor3 = THEME.SurfaceStrong,
        BackgroundTransparency = 1,
        Position = position,
        Size = size,
        Image = image,
    ...
    button.MouseEnter:Connect(function()
        tween(button, 0.16, {BackgroundTransparency = 0.2, ImageColor3 = THEME.Text})
    end)
    button.MouseLeave:Connect(function()
        tween(button, 0.16, {BackgroundTransparency = 1, ImageColor3 = THEME.Muted})
    end)
    button.MouseButton1Down:Connect(function()
        tween(button, 0.08, {ImageTransparency = 0.28})
    end)
    button.MouseButton1Up:Connect(function()
        tween(button, 0.1, {ImageTransparency = 0.02})
    end)
...
local backdrop = create("Frame", {
    ...
    BackgroundColor3 = Color3.fromRGB(4,3,5),
...
local main = create("Frame", {
    ...
    Size = UDim2.fromScale(0.90,0.86),
    ...
    corner(22),
    stroke(Color3.fromRGB(107,69,106), 0.38, 1),
    create("UISizeConstraint", {MinSize=Vector2.new(900,560), MaxSize=Vector2.new(1540,900)}),
    create("UIGradient", {
        Rotation = 18,
        Color = ColorSequence.new({
            ColorSequenceKeypoint.new(0, Color3.fromRGB(15,9,15)),
            ColorSequenceKeypoint.new(0.45, THEME.Background),
            ColorSequenceKeypoint.new(1, Color3.fromRGB(10,8,14)),
        }),
    }),
})
...
local innerGlow = create("Frame", {
    Name = "InnerGlow",
    Position = UDim2.fromOffset(-160,-210),
...
local brand = create("Frame", {
    ...
    Size = UDim2.fromOffset(225,92),
    Parent = header,
})

local brandIconWrap = create("Frame", {
    BackgroundColor3 = THEME.Purple,
    ...
    BorderSizePixel = 0,
    Parent = brand,
}, {corner(14), stroke(THEME.PurpleBright,0.52,1)})
...
create("TextLabel", {
    BackgroundTransparency = 1,
    Position = UDim2.fromOffset(57,26),
...
create("TextLabel", {
    ...
    Text = CONFIG.Tagline,
    TextColor3 = Color3.fromRGB(195,144,183),
...
local searchWrap = create("Frame", {
    Name = "SearchWrap",
    Position = UDim2.new(0,278,0,22),
    Size = UDim2.new(1,-482,0,50),
    BackgroundColor3 = Color3.fromRGB(19,15,20),
    BackgroundTransparency = 0.05,
    BorderSizePixel = 0,
    Parent = header,
}, {corner(15), stroke(Color3.fromRGB(102,71,103),0.38,1)})
...
local searchBox = create("TextBox", {
    ...
    PlaceholderText = "Search a script, game, or keyword...",
...
local countPill = create("Frame", {
    AnchorPoint = Vector2.new(1,0),
    Position = UDim2.new(1,-116,0,27),
    Size = UDim2.fromOffset(82,38),
    BackgroundColor3 = THEME.Surface,
    BackgroundTransparency = 0.12,
    BorderSizePixel = 0,
    Parent = header,
}, {corner(12), stroke(THEME.Stroke,0.55,1)})

local countLabel = create("TextLabel", {
    BackgroundTransparency = 1,
    Size = UDim2.fromScale(1,1),
    Text = tostring(#SCRIPT_LIBRARY) .. " scripts",
    TextColor3 = THEME.Muted,
    TextSize = 11,
    Font = Enum.Font.GothamMedium,
    Parent = countPill,
...
local filterClip = create("Frame", {
    BackgroundTransparency = 1,
    Position = UDim2.new(0,278,0,82),
    Size = UDim2.new(1,-320,0,40),
    ClipsDescendants = true,
    Parent = header,
})
...
local gridLayout = create("UIGridLayout", {
    ...
    CellSize = UDim2.fromOffset(205,185),
    SortOrder = Enum.SortOrder.LayoutOrder,
    FillDirection = Enum.FillDirection.Horizontal,
...
local footer = create("TextLabel", {
    ...
    Position = UDim2.new(0,30,1,-8),
    Size = UDim2.new(1,-60,0,16),
    BackgroundTransparency = 1,
    Text = "VELORA  •  RIGHT SHIFT TO TOGGLE",
...
local function notify(message, kind)
    ...
    local toast = create("Frame", {
        ...
        BorderSizePixel = 0,
        Parent = toastHolder,
    }, {corner(13),stroke(accent,0.46,1)})
...
local function runEntry(entry)
    if entry.Demo then
        notify(entry.Name .. " is a placeholder until we add your real script.", "success")
        return
    end
...
local function setFavorite(entry, value)
    ...
    if cardData then
        cardData.Favorite.ImageColor3 = value and THEME.Accent or THEME.Muted
        cardData.Favorite.ImageTransparency = value and 0 or 0.16
    end
end
...
local function createCard(entry, order)
    local card = create("Frame", {
        ...
        ClipsDescendants = true,
        Parent = grid,
    }, {corner(16),stroke(THEME.Stroke,0.5,1)})
    ...
    local art = create("Frame", {
        ...
        BackgroundColor3 = entry.A or THEME.Accent,
        BorderSizePixel = 0,
        ClipsDescendants = true,
        Parent = card,
    }, {
        ...
        create("UIGradient", {
            Rotation = 18,
            Color = ColorSequence.new({
                ColorSequenceKeypoint.new(0,entry.A or THEME.Accent),
                ColorSequenceKeypoint.new(1,entry.B or THEME.Purple),
            }),
        }),
    })
    ...
    create("Frame", {
        ...
        Position = UDim2.new(1,24,0.48,0),
    ...
    local iconHalo = create("Frame", {
        ...
        BorderSizePixel = 0,
        Parent = art,
    }, {corner(20),stroke(Color3.fromRGB(255,255,255),0.78,1)})
    ...
    create("TextLabel", {
        Name = "Subtitle",
        BackgroundTransparency = 1,
        Position = UDim2.fromOffset(12,125),
    ...
    local favorite = create("ImageButton", {
        Name = "Favorite",
        Position = UDim2.fromOffset(10,149),
    ...
    local more = create("ImageButton", {
        ...
        AutoButtonColor = false,
        Parent = card,
    }, {corner(9),create("UIPadding",{PaddingTop=UDim.new(0,7),PaddingBottom=UDim.new(0,7),PaddingLeft=UDim.new(0,7),PaddingRight=UDim.new(0,7)})})

    local open = create("TextButton", {
        Name = "Open",
        AnchorPoint = Vector2.new(1,1),
        Position = UDim2.new(1,-41,1,-8),
        Size = UDim2.fromOffset(76,28),
        BackgroundColor3 = Color3.fromRGB(63,40,61),
        BackgroundTransparency = 0.06,
        BorderSizePixel = 0,
        Text = "Open",
        TextColor3 = THEME.Text,
        ...
        AutoButtonColor = false,
        Parent = card,
    }, {corner(9),stroke(THEME.Accent,0.64,1)})
...
local function makeFilter(name, order)
    ...
    local button = create("TextButton", {
        ...
        AutoButtonColor = false,
        Parent = filters,
    }, {corner(16),stroke(THEME.Stroke,0.58,1)})
...
local function entryMatchesFilter(entry)
    ...
    if currentFilter == "Game Specific" then
        ...
        for _, tag in ipairs(entry.Tags or {}) do
            if normalize(tag) == "game specific" then return true end
        end
        return false
    end
...
local function entryMatchesSearch(entry, query)
    ...
    for _, tag in ipairs(entry.Tags or {}) do
        if contains(tag,query) then return true end
    end
    return false
end
...
local function refreshFilterStyles()
    for name, button in pairs(filterButtons) do
        ...
        tween(button,0.16,{
            ...
            TextColor3 = selected and Color3.fromRGB(35,17,30) or THEME.Muted,
        })
        if outline then
            tween(outline,0.16,{Color=selected and THEME.Accent or THEME.Stroke,Transparency=selected and 0.2 or 0.58})
        end
    end
end
...
for name, button in pairs(filterButtons) do
    ...
    button.MouseEnter:Connect(function()
        if currentFilter ~= name then
            tween(button,0.14,{BackgroundColor3=THEME.SurfaceStrong,TextColor3=THEME.Text})
        end
    end)
    button.MouseLeave:Connect(function()
        if currentFilter ~= name then
            tween(button,0.14,{BackgroundColor3=THEME.Surface,TextColor3=THEME.Muted})
        end
    end)
end
...
local function updateGridSize()
    ...
    local columns
    if width >= 1220 then columns = 6
    elseif width >= 990 then columns = 5
    elseif width >= 780 then columns = 4
    else columns = 3 end
...
if CONFIG.DemoMode then
    task.delay(0.55,function()
        if screen and screen.Parent then
            notify("Velora Hub refreshed with SaltyIcons.","success")
        end
    end)
end
